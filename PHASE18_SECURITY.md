...
npx vitest run                          # 333 tests (7 scale tier + 8 certification + 54 Phase 19)
...
2. ~~**No check-in system.**~~ **Closed in Phase 19.** `checkInOpensAt` / `checkInClosesAt` (nullable, admin-settable, derived from `registrationDeadline … startTime` when absent), `checkedInAt` / `noShowAt` on the registration, one guarded idempotent mutation, a live no-show pass on the server tick, a staff board, and a notification — plus Web Push for `MATCH_STARTING` / `ROOM_CREDENTIALS`. See `backend/PHASE19_NOTES.md`.
...
8. **Browser-level E2E exists but is not yet executed as evidence.** Phase 19 added `frontend/e2e/` (Playwright: smoke, the player journey through check-in, push opt-in honesty — 13 tests, collected and linted clean) and `backend/scripts/verify-journey.mjs` (`npm run verify:journey`), which runs the whole journey over HTTP against a real PostgreSQL. No browser binary could be installed in this environment, so no Playwright spec has run yet — see `frontend/e2e/README.md`. Service-level + HTTP-level coverage remains where the money invariants are proven.
